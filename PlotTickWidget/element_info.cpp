...
Element_Info::Element_Info(QObject *parent)
	: QObject(parent),
	filled(false),
	coord(0),
	error_flags(0)
...
void Element_Info::insertToDataStream( QDataStream& dataStream ) const
{
	   dataStream << QString("ELEMENT_INFO");
	DeviceSettings* devSettings = qApp->property("DeviceSettings").value<DeviceSettings*>();
	dataStream << filled << coord<<error_flags<<sum_defect<<chan_info_array.count();
 ...
 void Element_Info::extractFromDataStream( QDataStream& dataStream )
{
	DeviceSettings* devSettings = qApp->property("DeviceSettings").value<DeviceSettings*>();
		...
		case TD_DEF_PROD:
			{
				...
				dataStream>>(*defElement);
				chan_info_array.append(defElement);
			}
			break;
		case TD_DEF_POPER:
			{
 ...
 QDataStream& operator<<( QDataStream& dataStream, const Element_Info& element_info )
{
	element_info.insertToDataStream(dataStream);
	return dataStream;
}
QDataStream& operator>>( QDataStream& dataStream, Element_Info& element_info )
{
