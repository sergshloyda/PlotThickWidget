...
QString tstr;
	...
	_main_widget->show_ampl_info(tstr);
...
void ParamEditToolBox::apply_channals_states()
{
	...
	{
		const par_us_t& us=_curr_par_device->getChanAmpl(i);
		vec_state.push_back(us.on_us);
	}
	QList<QWidget *> widgets = _main_widget->findChildren<QWidget *>();
 	QList<QWidget *>::iterator it = std::find_if(widgets.begin(), widgets.end(),
    [](QWidget *widget) -> bool {
        return (qstrcmp(widget->metaObject()->className(),"ChanIndicWidget") == 0);
               
    });
	if(it!=widgets.end())
	{

		static_cast<ChanIndicWidget*>(*it)->SetChannelStates(vec_state);
	}
...
void ParamEditToolBox::chan_changed()
{
	...
	_main_widget->apply_chan_selection(new_chan,true);
	_main_widget->update_osc_widget();
...
void ParamEditToolBox::ninp_us_changed()
{
	...
	//_curr_par_device->setAmplMode(osc_par.ntact,osc_par.nus, chan_info.rej & TD_MASK);
...
void ParamEditToolBox::scale_mode_slot(bool scale_enabled)
{
	...
	_main_widget->update_chan_rows();
	//cn_list_t &cn_list = dev_obj.curr_par_device.contr_par.trk_par.cn_list;
	//for(quint8 i = 0; i < cn_list.chan_count; i++)
	//	ChanRowArr[i]->UpdatePlot();
}
