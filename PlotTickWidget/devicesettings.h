...
private:
	...
	std::map<quint16,QString> map_chan_name;
	std::map<QString,QColor> _incorrect_color;
	std::map<QString,QColor> _osc_color;
public:
	...
	const par_us_t& getChanAmpl(const quint8 nChan)
	{
	...
	const par_thick_t& getThickParams()
	{
	...
	const par_strb_t* getAmplStrobArray(const quint8 nChan) 
	{
		const cn_info_t &local_cn = _par_device.contr_par.trk_par.cn_list.cn_info[nChan];
	...
	const par_strb_info_t* getStrobInfoArray(const quint8 nChan)
	{
	...
	{
		return map_chan_name[getChanMode(nChan)];
	}
	const QColor& getIncorrectColor(const QString& err_name)
	{
		return _incorrect_color[err_name];
	}
	 quint8 getUsedLayers() const
	{
		return _par_device.contr_par.trk_par.cn_list.used_layers;
	}
	 QColor getOscColor(const QString& color_name) 
	{
		QColor white_color(Qt::white);
		if(_osc_color.count(color_name))
			return _osc_color[color_name];
		else
			return white_color;
	}
