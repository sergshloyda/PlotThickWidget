...
class Element_Info : public QObject
{
	Q_OBJECT

public:
	Element_Info(QObject *parent=0);
	~Element_Info();
	SumDefect_Info sum_defect;
	QVector<AbstractElement*> chan_info_array;
	bool filled;
	int coord;
	quint8 error_flags;
	void insertToDataStream( QDataStream& dataStream ) const;
	...
	friend QDataStream& operator<<( QDataStream& dataStream, const Element_Info& element_info );
    friend QDataStream& operator>>( QDataStream& dataStream, Element_Info& element_info );
