...
bool DataSerializer::readFile( QIODevice* ioDevice, par_device_t& device_params )
{
  ...
  bool DataSerializer::writeFile( QIODevice* ioDevice, const QList<Element_Info*>& elem_info_list )
  {
    ...
    bool DataSerializer::readFile( QIODevice* ioDevice, QList<Element_Info*>& elem_info_list )
	{
    ...
    if ( wasOpen || ioDevice->open( QIODevice::ReadOnly ) )
    {
		...
		while(count_elems--)
		{
			Element_Info* elem_info=new Element_Info();
        dataStream >> *elem_info;
