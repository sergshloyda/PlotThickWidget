...
bool c_base_dev_cmd::send_and_wait(const quint8 type, const quint8 cmd, const quint8 *in_dat, const quint16 in_len, quint8 *out_dat, quint16 *out_len)
{
	...
	gen_send_dat(type, cmd, in_dat, in_len);	// сгенерировать пакет данных
	...
	bool result = false;
	if(curr_cmd->ReseiveMutex.tryLock(timeout_resive))	// ответ пришёл вовремя
	{
		if(attached)
		{
			if(out_dat != nullptr)
